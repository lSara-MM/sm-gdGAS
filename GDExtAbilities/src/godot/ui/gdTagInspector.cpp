...
{
	m_Editor = editor;
	m_Editor->deleteTag.SubscribeEvent(this, &TagInspector::DeleteTags);
}

void sm::TagInspector::ShowTagTreeEditor(Inspect current, godot::Object* object, const godot::String& title)
{
	...
	if (current == Inspect::CONTAINER)
	{
		...
		addButton->connect("pressed", callable_mp(this, &TagInspector::_OnAddButtonClicked).bind(context.availableTree));
...
void sm::TagInspector::AddSearchControls(godot::BoxContainer* root, godot::Control* gui, TagTreeContext& context)
{
	...
	{
		search->connect("text_changed", callable_mp(this, &TagInspector::_OnSearchTextChanged).bind(context.availableTree));
		return;
	}
	...
	search->connect("text_changed", callable_mp(this, &TagInspector::_OnSearchTextChanged).bind(context.availableTree));
	showEnabled->connect("toggled", callable_mp(this, &TagInspector::_OnShowEnabledToggled).bind(context.availableTree));
	showDisabled->connect("toggled", callable_mp(this, &TagInspector::_OnShowDisabledToggled).bind(context.availableTree));
}

int sm::TagInspector::AddAvailableTagsTree(godot::VBoxContainer* root, TagTreeContext& context)
{
	...
	context.availableTree->connect("item_edited", callable_mp(this, &TagInspector::_OnCheckboxChanged).bind(context.availableTree));
	context.availableTree->connect("item_activated", callable_mp(this, &TagInspector::_OnItemActivated).bind(context.availableTree));
...
void sm::TagInspector::AddSelectedTagsTree(godot::Control* gui, godot::VSplitContainer* mainSplit, TagTreeContext& context)
{
	...
	context.selectedTree->connect("button_clicked", callable_mp(this, &TagInspector::_OnRemoveButtonClicked).bind(context.availableTree));
	mainSplit->add_child(context.selectedTree);
}

void sm::TagInspector::RefreshAvailableTree(TagTreeContext& context)
{
...
void sm::TagInspector::RefreshTagVisibility(TagTreeContext& context, const godot::Ref<TagData>& tag, godot::TreeItem* item)
{
...
std::vector<TagID> sm::TagInspector::SetCheckbox(TagTreeContext& context, godot::TreeItem* item, bool value)
{
	godot::Ref<TagData> tag = item->get_metadata(0);
...
godot::PackedInt32Array sm::TagInspector::GetTags(const TagTreeContext& context) const
{
...
void sm::TagInspector::SetTags(const TagTreeContext& context, const godot::PackedInt32Array& tags)
{
	switch (context.current)
	{
	case Inspect::EFFECT_ADD:
...
bool sm::TagInspector::HasTag(const TagTreeContext& context, TagID id) const
{
	if (context.current == Inspect::CONTAINER)
	{
		TagContainer* container = GetContainer(context);
...
void sm::TagInspector::AddTag(TagTreeContext& context, TagID id)
{
...
void sm::TagInspector::RemoveTag(TagTreeContext& context, TagID id)
{
	if (context.current == Inspect::CONTAINER)
	{
		if (TagContainer* container = GetContainer(context))
...
sm::TagInspector::TagTreeContext* sm::TagInspector::GetContext(godot::Tree* tree)
{
	auto it = m_Contexts.find(tree);
	return it == m_Contexts.end() ? nullptr : &it->second;
}

sm::TagContainer* sm::TagInspector::GetContainer(const TagTreeContext& context) const
{
	return godot::Object::cast_to<TagContainer>(godot::ObjectDB::get_instance(context.ownerId));
}

void sm::TagInspector::DeleteTags(const std::vector<godot::Ref<TagData>>& tags)
{
...
void sm::TagInspector::_OnCheckboxChanged(godot::Tree* tree)
{
...
void sm::TagInspector::_OnAddButtonClicked(godot::Tree* tree)
{
	TagTreeContext* context = GetContext(tree);
	ERR_FAIL_NULL(context);
...
void sm::TagInspector::_OnRemoveButtonClicked(godot::TreeItem* item, int column, int id, int mouseButtonIndex, godot::Tree* tree)
{
	TagTreeContext* context = GetContext(tree);
	ERR_FAIL_NULL(context);
...
void sm::TagInspector::_OnSearchTextChanged(const godot::String& text, godot::Tree* tree)
{
	TagTreeContext* context = GetContext(tree);
	ERR_FAIL_NULL(context);
...
void sm::TagInspector::_OnItemActivated(godot::Tree* tree)
{
	TagTreeContext* context = GetContext(tree);
	ERR_FAIL_NULL(context);
...
void sm::TagInspector::_OnShowEnabledToggled(bool toggled, godot::Tree* tree)
{
	TagTreeContext* context = GetContext(tree);
	ERR_FAIL_NULL(context);
...
void sm::TagInspector::_OnShowDisabledToggled(bool toggled, godot::Tree* tree)
{
	TagTreeContext* context = GetContext(tree);
	ERR_FAIL_NULL(context);
