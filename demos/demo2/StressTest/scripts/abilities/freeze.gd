...
func _end_ability(_was_cancelled: bool):
	...
	if is_instance_valid(target):
		var node = target.get_tag_container()
		print("HasTag Frozen (expected true): ", node.has_tag(Tags._State_Debuff_Frozen))
	return true
...
func _calculate_targets() -> Array[GAS_Entity]:
	...
	if target.get_tag_container().has_tag(Tags._Immunity_Freeze):
		return []
