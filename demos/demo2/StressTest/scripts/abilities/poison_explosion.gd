...
func _end_ability(_was_cancelled: bool) -> bool:
	print("End PoisonExplosion")
	print("Targets count: ", targets.size())
	if targets.is_empty():
		return true
	
	var node1 = targets[0].get_tag_container()
	print("HasTag Poisoned (should be false):", node1.has_tag(Tags._State_Debuff_Poisoned))
	var node = targets[0].get_attribute_container()
...
func _calculate_targets() -> Array[GAS_Entity]:
	...
	var arr: Array[Node] = world.all_with_tags([Tags._State_Debuff_Poisoned])
	targets = []
	print("PoisonExplosion")

	print("Targets found: ", targets.size())
	
	for node in arr:
		if node is GAS_Entity:
			var tag_node = node.get_tag_container()
